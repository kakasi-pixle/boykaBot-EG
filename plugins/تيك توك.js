...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  if (!text) throw `*استعمال خاطئ*\n*مثال* : \n _${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/_`;
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) throw `*استعمال خاطئ*\nمثال : \n _${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/_`;
  const texto = `*اصبر الفديو بيحمل متتصربعش 🗿*`;
  // let buttons = [{ buttonText: { displayText: '♫ 𝙰𝚄𝙳𝙸𝙾 ♫' }, buttonId: `${usedPrefix}tomp3` }]
  try {
    const aa = {quoted: m, userJid: conn.user.jid};
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: '𝐁𝐎𝐀𝐘𝐊𝐀', body: null, thumbnail: imagen1, sourceUrl: 'https://whatsapp.com/channel/0029ValRCUIFi8xmK9xBOr0I'}, mentionedJid: [m.sender]}}}, aa);
    ...
    const desc1n = `*خد اهو الفديو اجري بقي 🗿*`;
  ...
  try {
    ...
    const desc1 = `*خد اهو الفديو اجري بقي 🗿*`;
