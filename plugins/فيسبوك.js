...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  ...
  if (!args[0]) {
    throw `*خطاء 💀*\n*مثال :*\n${usedPrefix + command} https://fb.watch/fOTpgn6UFQ/_`;
  }

  /*const linkface = await isValidFacebookLink(args[0]);
  if (!linkface) {
    throw `*خد اهو الفديو اجري متصدعنيش 🗿*${usedPrefix + command} https://fb.watch/fOTpgn6UFQ/_`;
  }*/

  if (!enviando) enviando = true;
  try {
    await m.reply(`*اصبر ياض الفديو بيحمل 🗿*`);
    ...
    if (data?.status === true) {
      ...
      await conn.sendMessage(m.chat, {
        video: videoBuffer, 
        filename: 'video.mp4', 
        caption: `*خد اهو الفديو اجري متصدعنيش 🗿*`
      }, {quoted: m});
      enviando = false;
    } else {
...
handler.command = /^(facebook|fb|فيس بوك|فيسبوك|فيس|fb2|facebookdl2|fbdl2|facebook3|fb3|facebookdl3|fbdl3|facebook4|fb4|facebookdl4|fbdl4|facebook5|fb5|facebookdl5|fbdl5)$/i;
